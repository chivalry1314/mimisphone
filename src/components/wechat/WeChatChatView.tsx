...
export const WeChatChatView: React.FC<WeChatChatViewProps> = ({ characterId, onBack }) => {
  const { 
    ...
    createWeChatSession, settings, worldBook, wechatUserProfile, withdrawWeChatBalance
  } = useStore();
  ...
  const executeForward = (targetCharacterId: string) => {
    ...
    if (!targetSessionId) {
      ...
      useStore.setState((state) => ({ wechatSessions: [{ id: targetSessionId!, characterId: targetCharacterId, messages: [], lastUpdated: Date.now(), unreadCount: 0 }, ...state.wechatSessions] }));
    }
  ...
  const buildApiMessages = (basePrompt: string, sessionId: string) => {
    const chatHistory = useStore.getState().wechatSessions.find(s => s.id === sessionId)?.messages || [];
